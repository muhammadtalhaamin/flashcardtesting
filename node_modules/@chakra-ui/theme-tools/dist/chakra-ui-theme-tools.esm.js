...
const getColor = (theme, color, fallback) => {
  const hex = memoizedGet(theme, `colors.${color}`, color);
  const {
    isValid
  } = new TinyColor(hex);
...
const lighten = (color, amount) => theme => new TinyColor(getColor(theme, color)).lighten(amount).toHexString();
/**
...
const contrast = (fg, bg) => theme => readability(getColor(theme, bg), getColor(theme, fg));
/**
...
const complementary = color => theme => new TinyColor(getColor(theme, color)).complement().toHexString();
...
class Anatomy {
   ...
   */
  get selectors() {
    const value = fromEntries(Object.entries(this.map).map(([key, part]) => [key, part.selector]));
    return value;
  }
  /**
  ...
  get classNames() {
    const value = fromEntries(Object.entries(this.map).map(([key, part]) => [key, part.className]));
    return value;
  }
  /**
